...
void eval_micro_sequencer() {
  ...
  {
    nextstate = (int)((unsigned int)CURRENT_LATCHES.IR >> 8);
  }
  else
  {
...
if (GetLD_REG(CURRENT_LATCHES.MICROINSTRUCTION))
{
    if(GetDRMUX(CURRENT_LATCHES.MICROINSTRUCTION))
        CURRENT_LATCHES.REGS[7] = BUS;
    else
        CURRENT_LATCHES.REGS[(int)((unsigned int)CURRENT_LATCHES.IR >> 9)&7] = BUS; 
}
if (GetLD_IR(CURRENT_LATCHES.MICROINSTRUCTION))
{
    CURRENT_LATCHES.IR = BUS;
}
