...
void MEM_stage() {
  ...
  if (Get_MEM_LD_CC(PS.MEM_CS) && PS.MEM_V)
    v_mem_ld_cc = 1;
  else
    v_mem_ld_cc = 0;
  if (Get_MEM_BR_STALL(PS.MEM_CS) && PS.MEM_V)
    v_mem_br_stall = 1;
  else
    v_mem_br_stall = 0;
  if (Get_MEM_LD_REG(PS.MEM_CS) && PS.MEM_V)
    v_mem_ld_reg = 1;
  else
    v_mem_ld_cc = 0;
  target_pc = PS.MEM_ADDRESS; 
  if (Get_BR_OP(PS.MEM_CS) && (((PS.MEM_IR&3584)>>9)&PS.MEM_CC) && PS.MEM_V)
  {
    takeit = 1;
    mem_pcmux = 1;
  }
  else if (Get_UNCOND_OP(PS.MEM_CS) && PS.MEM_V)
  {
    takeit = 1;
    mem_pcmux = 1;
  }
  else if (Get_TRAP_OP(PS.MEM_CS) && PS.MEM_V)
  {
    takeit = 1;
    mem_pcmux = 2;
  }
  ...
  if (Get_DCACHE_RW(PS.MEM_CS) && PS.MEM_V)
  {
  ...
  {
    int writein = PS.MEM_ALU_RESULT;
    if(Get_DATA_SIZE(PS.MEM_CS) == 0)
    {
      if(PS.MEM_ADDRESS&1)
        writein = writein << 8;
    }
    dcache_access(PS.MEM_ADDRESS, &readed, writein , &dcache_r, we0, we1);
    if(Get_DATA_SIZE(PS.MEM_CS) == 0)
    {
      if(PS.MEM_ADDRESS&1)
        readed = signext((int16_t)((uint16_t)readed >> 8),8);
      else
        readed = signext(readed,8);
    }
    NEW_PS.SR_DATA = readed;
    trap_pc = readed;
  }
  else
    NEW_PS.SR_DATA = 0;
  ...
  if (dcache_r == 0 && Get_DCACHE_EN(PS.MEM_CS) && PS.MEM_V)
    mem_stall = 1;
  else 
    mem_stall = 0;
  if((Get_DCACHE_EN(PS.MEM_CS) && dcache_r == 0 && PS.MEM_V) || PS.MEM_V == 0 || (mem_stall == 1 && PS.MEM_V))
    NEW_PS.SR_V = 0;
  else
    NEW_PS.SR_V = 1;
...
void AGEX_stage() {
  ...
  int16_t addr1;
  int16_t addr2;
  int16_t alu_op1 = PS.AGEX_SR1;
  int16_t alu_op2;
  int16_t alu_result;
  int16_t shift_result;
  if (Get_SR2MUX(PS.AGEX_CS))
    alu_op2 = signext(PS.AGEX_IR,5);
  else
    alu_op2 = PS.AGEX_SR2;
  if (Get_AGEX_LD_CC(PS.AGEX_CS) && PS.AGEX_V)
    v_agex_ld_cc = 1;
  else
    v_agex_ld_cc = 0;

  if (Get_AGEX_BR_STALL(PS.AGEX_CS)&&PS.AGEX_V)
    v_agex_br_stall = 1;
  else 
    v_agex_br_stall = 0;
  
  if (Get_AGEX_LD_REG(PS.AGEX_CS)&&PS.AGEX_V)
    v_agex_ld_reg = 1;
  else
    v_agex_ld_reg = 0;  
  ...
  if (LD_MEM) {
    ...
    if (Get_ADDR2MUX(PS.AGEX_CS) == 0)
      ...
      addr2 = signext(PS.AGEX_IR,6);
    else if (Get_ADDR2MUX(PS.AGEX_CS) == 2)
      addr2 = signext(PS.AGEX_IR,9);
    else if (Get_ADDR2MUX(PS.AGEX_CS) == 3)
      addr2 = signext(PS.AGEX_IR,11);
    if (Get_LSHF1(PS.AGEX_CS))
      addr2 = addr2 << 1;
    if (Get_ADDRESSMUX(PS.AGEX_CS))
      NEW_PS.MEM_ADDRESS = (uint16_t)(addr2 + addr1);
    else
      NEW_PS.MEM_ADDRESS = (uint16_t)((PS.AGEX_IR)&255) << 1;
    ...
    if((PS.AGEX_IR&48) == 48)
      shift_result = (int16_t)PS.AGEX_SR1 >> (PS.AGEX_IR&15);
    else if ((PS.AGEX_IR&48) == 16) 
      shift_result = (int16_t)((uint16_t)PS.AGEX_SR1 >> (PS.AGEX_IR&15));
   else
      shift_result = PS.AGEX_SR1 << (PS.AGEX_IR&15);
    if(Get_ALU_RESULTMUX(PS.AGEX_CS))
      NEW_PS.MEM_ALU_RESULT = (uint16_t)alu_result;
    else
      NEW_PS.MEM_ALU_RESULT = (uint16_t)shift_result;
...
void DE_stage() {
  ...
  CONTROL_STORE_ADDRESS = (PS.DE_IR&63488)>>10;
  if (PS.DE_IR&32)
    CONTROL_STORE_ADDRESS = CONTROL_STORE_ADDRESS + 1;
  int ii, jj = 0;
  ...
  if (PS.DE_IR&8192)
      ...
      SR2 = (PS.DE_IR&7);
  dep_stall = 0;
  if (((v_agex_ld_cc&& PS.AGEX_V)||(v_mem_ld_cc&& PS.MEM_V)||(v_sr_ld_cc && PS.SR_V)) && Get_DE_BR_OP(CONTROL_STORE[CONTROL_STORE_ADDRESS])&& PS.DE_V)
    dep_stall = 1;
  if(Get_SR1_NEEDED(CONTROL_STORE[CONTROL_STORE_ADDRESS])&& PS.DE_V)
  {
    if ((v_mem_ld_reg && ((PS.MEM_DRID == (PS.DE_IR&448)>>6)) && PS.MEM_V) ||
  (v_agex_ld_reg && ((PS.AGEX_DRID == (PS.DE_IR&448)>>6)) && PS.AGEX_V) ||
  (v_sr_ld_reg && ((PS.SR_DRID == (PS.DE_IR&448)>>6)) && PS.DE_V))
    dep_stall = 1;
  }
  if(Get_SR2_NEEDED(CONTROL_STORE[CONTROL_STORE_ADDRESS])&& PS.DE_V)
  {
    if ((v_mem_ld_reg && (PS.MEM_DRID == SR2) && PS.MEM_V) ||
  (v_agex_ld_reg && (PS.AGEX_DRID == SR2) && PS.AGEX_V) ||
  (v_sr_ld_reg && (PS.SR_DRID == SR2) && PS.DE_V))
    dep_stall = 1;
  }
  ...
  if (PS.DE_V && CONTROL_STORE[CONTROL_STORE_ADDRESS][15])
      v_de_br_stall = 1;
    else
      v_de_br_stall = 0;
  ...
  if (LD_AGEX) {
    ...
    if (dep_stall || PS.DE_V == 0)
    {
      NEW_PS.AGEX_V = 0;
    }
    else 
      NEW_PS.AGEX_V = 1;
    NEW_PS.AGEX_SR1 = REGS[(PS.DE_IR&448)>>6];
    ...
    if (Get_DRMUX(CONTROL_STORE[CONTROL_STORE_ADDRESS]))
      ...
      NEW_PS.AGEX_DRID = (PS.DE_IR&3584)>>9;
...
void FETCH_stage() {
  ...
  icache_access(PC,&IRlatch,&icache_r);
  ...
  if(dep_stall||mem_stall)
  {
    ...
    loade = 1;
  if (icache_r && !v_de_br_stall && !v_mem_br_stall && !v_agex_br_stall)
  {
    valid = 1;
  }
  else
  ...
  {
    NEW_PS.DE_NPC = PC+2;
    NEW_PS.DE_IR = IRlatch;
    NEW_PS.DE_V = valid;
  }

  if(icache_r && !(dep_stall||mem_stall||v_de_br_stall||v_mem_br_stall||v_agex_br_stall))
    PC = PC + 2;
  ...
  /*
  if ((dep_stall||mem_stall) && !v_de_br_stall && !v_mem_br_stall && !v_agex_br_stall)
  {
    NEW_PS.DE_NPC = PC;
    NEW_PS.DE_V = 1;
  }
  else if (icache_r && !v_de_br_stall && !v_mem_br_stall && !v_agex_br_stall)
  {
    NEW_PS.DE_NPC = PC + 2;
